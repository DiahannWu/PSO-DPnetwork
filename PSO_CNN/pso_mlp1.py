...
def tra_model(theWight):
    ...
    for c, data in enumerate(train_loader, 1):
        ...
        # print("当前损失函数为：", loss)
    theWight[0, :, :] = model.layer1.weight.data.cpu().numpy()
    theWight[1, 0:100, 0:300] = model.layer2.weight.data.cpu().numpy()
    theWight[2, 0:10, 0:100] = model.layer3.weight.data.cpu().numpy()
...
new_weight = np.zeros((num, numx, numy, numz))
...
for i in range(max_iter):
    ...
    loss, correct, total, accuracy, _ = tra_model(allbest_P)
    print('loss:', loss.item())
    print('correct: ', correct)
    print('total: ', total)
