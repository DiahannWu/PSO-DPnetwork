...
n_weight2 = [64, 32*3*3]
n_weight3 = [10, 2304]
all_weight = n_weight1[0]*n_weight1[1]+n_weight1[0]+n_weight2[0]*n_weight2[1]+n_weight2[0]+n_weight3[0]*n_weight3[1]+n_weight3[0]
n1 = n_weight1[0]*n_weight1[1]
n2 = n1 + n_weight1[0]
n3 = n2 + n_weight2[0]*n_weight2[1]
n4 = n3 + n_weight2[0]
n5 = n4 + n_weight3[0]*n_weight3[1]
n6 = n5 + n_weight3[0]
...
parser.add_argument('--n_epoch', default=1000, type=int, help='total number of epochs')
...
parser.add_argument('--eps', default=10., type=float, help='privacy parameter epsilon')
parser.add_argument('--delta', default=1e-5, type=float, help='desired delta')

parser.add_argument('--result_dir', default='fashion_result/cnn', type=str, help='reslult file dir')
parser.add_argument('--file_name', default='fashion_result_clip1_eps10_debugg', type=str, help='reslult file name')
...
def tra_model(theWight):
    ...
    net.convl[0].weight.data = torch.from_numpy(theWight[0:n1].reshape(n_weight1[0], 1, 3, 3)).to(torch.float32).cuda()
    net.convl[0].bias.data = torch.from_numpy(theWight[n1:n2].reshape(n_weight1[0])).to(torch.float32).cuda()
    net.convl[2].weight.data = torch.from_numpy(theWight[n2:n3].reshape(n_weight2[0], 32, 3, 3)).to(torch.float32).cuda()
    net.convl[2].bias.data = torch.from_numpy(theWight[n3:n4].reshape(n_weight2[0])).to(torch.float32).cuda()
    net.dense[0].weight.data = torch.from_numpy(theWight[n4:n5].reshape(n_weight3[0], n_weight3[1])).to(torch.float32).cuda()
    net.dense[0].bias.data = torch.from_numpy(theWight[n5:n6].reshape(n_weight3[0])).to(torch.float32).cuda()
    ...
    for c, data in enumerate(train_loader, 1):
        ...
        # print("当前损失函数为：", loss)
    theWight[0: n1] = net.convl[0].weight.data.cpu().numpy().flatten()[:]
    theWight[n1: n2] = net.convl[0].bias.data.cpu().numpy().flatten()[:]
    theWight[n2: n3] = net.convl[2].weight.data.cpu().numpy().flatten()[:]
    theWight[n3: n4] = net.convl[2].bias.data.cpu().numpy().flatten()[:]
    theWight[n4: n5] = net.dense[0].weight.data.cpu().numpy().flatten()[:]
    theWight[n5: n6] = net.dense[0].bias.data.cpu().numpy().flatten()[:]
...
# p为粒子位置矩阵，初始为标准正态分布
p = np.random.randn(num, all_weight)

# v为粒子速度矩阵，初始为标准正态分别
v = np.random.randn(num, all_weight)
