...
n_weight1 = 10
n_weight2 = 100
all_weight = n_weight1*n_weight2+n_weight1
...
def tra_model(theWight):
    ...
    model.layer3.weight.data = torch.from_numpy(theWight[0:n_weight1*n_weight2].reshape(n_weight1, n_weight2)).to(torch.float32).cuda()
    model.layer3.bias.data = torch.from_numpy(theWight[n_weight1*n_weight2:all_weight].reshape(n_weight1)).to(torch.float32).cuda()
    ...
    for c, data in enumerate(train_loader, 1):
        ...
        # print("当前损失函数为：", loss)
    theWight[0: n_weight1*n_weight2] = model.layer3.weight.data.cpu().numpy().flatten()[:]
    theWight[n_weight1*n_weight2: all_weight] = model.layer3.bias.data.cpu().numpy().flatten()[:]
