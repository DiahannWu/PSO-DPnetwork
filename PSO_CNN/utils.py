...
def process_batch(params, clipping=1):
    n = 1
    norm_list = torch.zeros(n).to(device)
    for p in params:
        # print(p.shape)
        flat_g = p.reshape(n, -1)
        current_norm_list = torch.norm(flat_g, dim=1)
        norm_list += torch.square(current_norm_list)
    norm_list = torch.sqrt(norm_list)
    scaling = clipping/norm_list
    scaling[scaling>1] = 1

    for p in params:
        p_dim = len(p.shape)
        scaling = scaling.view([1]*p_dim)
        p *= scaling
...
def l2_regularization(model):
    ...
    for module in model.modules():
        if (type(module) is nn.Conv2d) | (type(module) is nn.Linear):
